...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function clearError(field) {
      ...
      if (nextSibling && nextSibling.classList && nextSibling.classList.contains('form-error-message')) {
        nextSibling.remove();
      }
    }
      ...
      if (value !== '') {
        ...
        if (field.pattern) {
          ...
          if (!pattern.test(value)) {
            const patternTitle = field.title || 'Please match the required format.';
            showError(field, patternTitle);
            return false;
          }
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const submitFields = contactForm.querySelectorAll('input, textarea, select');
      submitFields.forEach(function (field) {
        if (field.type === 'submit' || field.type === 'button' || field.type === 'reset' || field.type === 'hidden') return;
  ...
  const animatableElements = document.querySelectorAll(animationSelectors.join(', '));
  ...
  if (animatableElements.length > 0) {
    ...
    animatableElements.forEach(function (el) {
      ...
      el.setAttribute('data-scroll-animate', 'pending');

      if (el.classList.contains('fade-in-up') || el.classList.contains('slide-up')) {
    ...
    function revealElement(el) {
      ...
      el.setAttribute('data-scroll-animate', 'done');
    ...
    if ('IntersectionObserver' in window) {
      ...
      function checkScrollFallback() {
        ...
        animatableElements.forEach(function (el) {
          if (el.getAttribute('data-scroll-animate') === 'done') return;
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '126e2db7-94ad-4a8a-8cd1-8bf58e789e27',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '126e2db7-94ad-4a8a-8cd1-8bf58e789e27';
...
/* Lightweight lightbox-style zoom on click */
(function () {
  ...
  lightbox.setAttribute('aria-label', 'תמונה מוגדלת');
  lightbox.classList.add('gallery-lightbox');

  const lbImg = document.createElement('img');
  lbImg.classList.add('gallery-lightbox__img');
  lbImg.alt = '';

  const lbClose = document.createElement('button');
  lbClose.classList.add('gallery-lightbox__close');
  lbClose.setAttribute('aria-label', 'סגור');
  ...
  // Inject lightbox CSS dynamically
  ...
  style.textContent = `
    ...
    .gallery-lightbox__img {
      ...
      object-fit: contain;
      border-radius: 10px;
      box-shadow: 0 8px 60px rgba(0,212,255,0.25);
...
/* Added Component Script */
document.querySelectorAll('.maor-video-card__thumbnail-wrapper').forEach(wrapper => {
  wrapper.addEventListener('click', function(e) {
    if (e.target.closest('.maor-video-card__play-btn')) {
      return;
    }
    const playBtn = this.querySelector('.maor-video-card__play-btn');
    if (playBtn) {
      playBtn.click();
    }
  });
...
document.querySelectorAll('.maor-video-card__play-btn').forEach(btn => {
  btn.addEventListener('click', function(e) {
    e.stopPropagation();
    const card = this.closest('.maor-video-card');
    const title = card.querySelector('.maor-video-card__title')?.textContent || 'וידאו';
    console.log('מפעיל וידאו:', title);
    alert('וידאו: ' + title + '\n(יש להחליף בקישור אמיתי לוידאו)');
  });
});
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '126e2db7-94ad-4a8a-8cd1-8bf58e789e27';
